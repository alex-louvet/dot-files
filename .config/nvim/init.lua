...
vim.api.nvim_create_user_command('TypstPin', function()
  ...
  for _, client in pairs(clients) do
    if client.name == 'tinymist' then
      return client:exec_cmd { title = 'TypstPin', command = 'tinymist.pinMain', arguments = { vim.api.nvim_buf_get_name(0) } }
    end
  end
...
-- Move commands
vim.keymap.set('v', 'J', ":m '>+1<CR>gv=gv", { desc = 'Move selected line below' })
vim.keymap.set('v', 'K', ":m '<-2<CR>gv=gv", { desc = 'Move selected line below' })
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      local servers = {
        ...
        --         -- linter options
        --         pylint = { enabled = true, executable = 'pylint', args = { '--disable=F0401' } },
      ...
      for server_name, server_config in pairs(self_managed_install) do
        server_config.capabilities = vim.tbl_deep_extend('force', {}, capabilities, server_config.capabilities or {})
        vim.lsp.enable(server_name)
        vim.lsp.config(server_name, server_config)
      end
